...
pure nothrow @property bool shouldRead(HttpBodyTransmissionMode mode)
{
...
public alias HttpParserDelegate = void delegate(HttpParser);
public alias HttpParserBodyChunkDelegate = void delegate(HttpParser, HttpBodyChunk);
public alias HttpParserStringDelegate = void delegate(HttpParser, string data);
public alias HttpParserHeaderDelegate = void delegate(HttpParser, HttpHeader header);
public class HttpParser
{
	private 
	{
		...
		const __gshared int CB_OK = 0;
		const __gshared int CB_ERR = 1;
		int _headerFields;
		int _headerValues;
		uint _statusCode;
